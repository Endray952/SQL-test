...
class DataBaseManager(context: Context) {
    ...
    fun testSearchWithID(){
        val thread = Thread {
            var sum_time: Long = 0
            for (i in 1..1000) {
                val rnd = (1..SIZE).random()
                val startTime = System.nanoTime()
                db?.rawQuery("SELECT * FROM ${DataBaseConsts.TestTable.TABLE_NAME} WHERE ${DataBaseConsts.TestTable.ID} = $rnd", null)
                val endTime = System.nanoTime()
                sum_time += (endTime - startTime)
                //Log.d("MyLog", "testSearchWithID: ${endTime - startTime}")
            }
            Log.d("MyLog", "testSearchWithID: ${(sum_time/1000)}")
        }
        thread.start()
    }
    fun testSearchWithNonKey(){
        val thread = Thread {
            ...
            for (i in 1..100) {
                ...
                db?.rawQuery("SELECT * FROM ${DataBaseConsts.TestTable.TABLE_NAME} WHERE ${DataBaseConsts.TestTable.COLUMN_NAME_RANDOM_INT} = $rnd", null)
                var endTime = System.nanoTime()
                sum_time += (endTime - startTime)
            }
            Log.d("MyLog", "testSearchWithNonKey: ${(sum_time/100)}")
        }
        thread.start()
    }
    fun testSearchWithMask(){
        val thread = Thread {
            var sum_time: Long = 0
            for (i in 1..100) {
                var rnd = "str${(1..SIZE).random()}"
                var startTime = System.nanoTime()
                db?.rawQuery("SELECT * FROM ${DataBaseConsts.TestTable.TABLE_NAME} WHERE ${DataBaseConsts.TestTable.COLUMN_NAME_TEXT} LIKE '%2'", null)
                var endTime = System.nanoTime()
                sum_time += (endTime - startTime)
            }
            Log.d("MyLog", "testSearchWithMask: ${(sum_time/100)}")
        }
        thread.start()
    }
    fun testAddItem(){
    ...
    fun testAddGroupOfItems_1(group_size: Int = 10){
        val thread = Thread {
            var sum_time: Long = 0
            for (i in 1..100) {
                val str = "str$i"
            ...
            Log.d("MyLog", "testAddGroupOfItems_1: ${(sum_time/100)}")
        }
        thread.start()
